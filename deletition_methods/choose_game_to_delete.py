...
def choose_game_to_delete(message):
    ...
    if not games:
        bot.getBot().send_message(message.chat.id, "В этой категории нет игр для удаления.", reply_markup=m.main_menu())
        return
    ...
    msg = bot.getBot().send_message(message.chat.id, f"Выберите игру для удаления из категории '{category}':",
                           reply_markup=markup)
    bot.getBot().register_next_step_handler(msg, delete_game, category)